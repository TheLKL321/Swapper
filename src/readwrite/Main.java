...
public class Main {
    ...
    public static final int[] book = new int[20];

    public static int readerCount = 0, writerCount = 0, readersWaiting = 0, writersWaiting = 0;
    private static final Swapper<Boolean> mutex = new Swapper<>();
    public static final Swapperphore readerSwapperphore = new Swapperphore(0), writerSwapperphore = new Swapperphore(0);

    public static void acquireMutex() throws InterruptedException {
        HashSet<Boolean> temp = new HashSet<>();
        temp.add(true);
        mutex.swap(temp, new HashSet<>());
    }

    public static void releaseMutex() throws InterruptedException {
        HashSet<Boolean> temp = new HashSet<>();
        temp.add(true);
